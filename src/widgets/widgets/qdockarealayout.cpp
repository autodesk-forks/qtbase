...
bool QDockAreaLayout::restoreDockWidget(QDockWidget *dockWidget)
{
    ...
    if (placeHolder->window) {
        ...
        //-------------------------------------------------------------------------
        // Autodesk 3ds Max Change: For the 'unplug' parameter we pass in 'false' instead 
        // of 'true'. Same as when the dockwidget is made floating. Otherwise the 
        // QWidgetResizeHandler won't be activated when a floating window gets restored,
        // and the user can't resize the docking window.
        //-------------------------------------------------------------------------
        dockWidget->d_func()->setWindowState(true, false, r);
    }
                                                ...
                                                const QPoint &dest)
{
    ...
    if ( doExtendedDockWidgetResize( mainWindow ) )
    {
        ...
        if ( info )
        {
            ...
            if ( separator.size() == 1 ) // resize on a center separator
            ...
            if ( delta != 0 )
            {
                ...
                if ( info->o == dockAreaOrientation || separator.size() == 1 )
                {
